    ...
    <div className="space-y-6">
                   ...
                   <div className="flex-1 w-full sm:w-56">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="appearance-none w-full bg-gray-900 text-gray-200 rounded px-3.5 pr-10 py-2.5 border border-gray-600 hover:border-gray-500 focus:outline-none focus:ring-2 focus:ring-primary"
                        >
            ...
            <Link to={`/positions/${pos.id}`} key={pos.id} className="bg-gray-800 rounded-lg sm:rounded-xl p-3 sm:p-4 flex justify-between items-center hover:bg-gray-700/60 transition">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${pos.side === Side.LONG ? 'bg-accent/20 text-accent' : 'bg-red-900 text-red-300'}`}>{pos.side}</span>
              ...
              <div className={`text-right text-sm font-semibold ${pos.pnl_gbp === null ? 'text-gray-400' : pos.pnl_gbp >= 0 ? 'text-accent' : 'text-red-400'}`}>
      ...
      <div className="card-premium rounded-lg shadow-lg overflow-x-auto hidden md:block">
        <table className="min-w-full table-premium">
          <thead>
            <tr>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Symbol</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Side</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Entry Time</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Entry Price</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Method</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">P&L</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Status</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-gray-400">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-gray-800 divide-y divide-gray-700">
            {filteredPositions.map(pos => (
              <tr key={pos.id} className="hover:bg-gray-700/50">
                ...
                <td className={`px-4 py-3 whitespace-nowrap text-sm font-semibold ${pos.side === Side.LONG ? 'text-accent' : 'text-red-400'}`}>{pos.side}</td>
                ...
                <td className={`px-4 py-3 whitespace-nowrap text-sm font-medium ${pos.pnl_gbp === null ? 'text-gray-400' : pos.pnl_gbp >= 0 ? 'text-accent' : 'text-red-400'}`}>{pos.pnl_gbp !== null ? pos.pnl_gbp.toFixed(2) : 'N/A'}</td>
