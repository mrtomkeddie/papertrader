    ...
    <div className="space-y-6">
                   ...
                   <div className="flex-1 w-full sm:w-56">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="appearance-none w-full searchbar rounded px-3.5 pr-10 py-2.5 focus:outline-none focus:ring-2 focus:ring-[rgba(16,185,129,0.35)]"
                        >
            ...
            <Link to={`/positions/${pos.id}`} key={pos.id} className="card-premium rounded-lg sm:rounded-xl p-3 sm:p-4 flex justify-between items-center hover:bg-[rgba(24,24,24,0.72)] hover:ring-1 hover:ring-white/10 transition">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${pos.side === Side.LONG ? 'bg-[rgba(16,185,129,0.15)] text-accent-green' : 'bg-red-900 text-red-300'}`}>{pos.side}</span>
              ...
              <div className={`text-right text-sm font-semibold ${pos.pnl_gbp === null ? 'text-gray-400' : pos.pnl_gbp >= 0 ? 'text-accent-green' : 'text-red-400'}`}>
      ...
      <div className="card-premium rounded-lg shadow-lg overflow-x-auto hidden md:block">
        <table className="min-w-full table-premium">
          <thead>
            <tr>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Symbol</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Side</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Entry Time</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Entry Price</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Method</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">P&L</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Status</th>
              <th className="px-4 py-3 text-left text-[11px] tracking-wide text-text-secondary">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-transparent divide-y divide-white/10">
            {filteredPositions.map(pos => (
              <tr key={pos.id} className="hover:bg-[rgba(24,24,24,0.6)]">
                ...
                <td className={`px-4 py-3 whitespace-nowrap text-sm font-semibold ${pos.side === Side.LONG ? 'text-accent-green' : 'text-red-400'}`}>{pos.side}</td>
                ...
                <td className={`px-4 py-3 whitespace-nowrap text-sm font-medium ${pos.pnl_gbp === null ? 'text-gray-400' : pos.pnl_gbp >= 0 ? 'text-accent-green' : 'text-red-400'}`}>{pos.pnl_gbp !== null ? pos.pnl_gbp.toFixed(2) : 'N/A'}</td>
