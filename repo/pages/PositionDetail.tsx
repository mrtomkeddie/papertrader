...
const PositionDetail: React.FC = () => {
  ...
  const pnlColor = position.pnl_gbp === null ? 'text-gray-400' : position.pnl_gbp >= 0 ? 'text-accent-green' : 'text-red-400';
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-3">
        ...
        <span className={`self-start sm:self-auto px-2 py-0.5 text-xs font-semibold rounded-full ${position.status === PositionStatus.OPEN ? 'bg-[rgba(255,255,255,0.06)] text-gray-200' : 'bg-[rgba(255,255,255,0.12)] text-gray-300'}`}>
            {position.status}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="card-premium p-3 sm:p-6 rounded-lg sm:rounded-xl shadow-lg grid grid-cols-2 md:grid-cols-4 gap-x-3 sm:gap-x-6 gap-y-4 sm:gap-y-8">
            <DetailItem label="Side" value={position.side} color={position.side === Side.LONG ? 'text-accent-green' : 'text-red-400'} />
      ...
      <div className="card-premium chart-gloss rounded-lg sm:rounded-xl shadow-lg h-[320px] sm:h-[420px] lg:h-[500px]">
