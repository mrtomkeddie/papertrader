import { Bot } from './types';
import { fetchOHLCV } from '../services/dataService';
import { evaluateTrendPullback } from '../strategies/trendPullback';
import { TIMEFRAME_BY_SYMBOL } from '../constants';
import { calculateATR } from '../strategies/indicators';

const SYMBOL = 'OANDA:NAS100_USD';
const TF = TIMEFRAME_BY_SYMBOL[SYMBOL] || '15m';

export const trendAtrBotNas: Bot = {
  id: 'trendAtr',
  isEnabled() {
    const list = (process.env.VITE_ENABLED_BOTS || process.env.AUTOPILOT_ENABLED_BOTS || '').toLowerCase();
    return list ? list.split(',').map(s => s.trim()).includes('trendatr') : true;
  },
  isWindowOpen(now: Date) {
    const h = now.getUTCHours();
    const d = now.getUTCDay();
    return d >= 1 && d <= 5 && h >= 12 && h < 20;
  },
  async scan() {
    const ohlcv = await fetchOHLCV(SYMBOL, TF, 200);
    if (ohlcv.length >= 20) {
      const atrSeries = calculateATR(ohlcv, 14);
      const latestATR = atrSeries[atrSeries.length - 1];
      const latestClose = ohlcv[ohlcv.length - 1]?.close;
      if (Number.isFinite(latestATR) && Number.isFinite(latestClose)) {
        const atrPct = (latestATR / latestClose) * 100;
        if (atrPct < 0.1 || atrPct > 2.5) { // NAS100 tolerance band
          return [];
        }
      }
    }
    const sig = evaluateTrendPullback(ohlcv);
    return sig ? [{ ...sig, strategy: 'Trend Pullback' } as any] : [];
  },
  selectSignals(candidates) {
    const minRR = 1.0;
    const chosen = candidates.filter(s => (s.rrr ?? 0) >= minRR);
    return chosen.map(s => ({
      side: s.side,
      entry_price: s.entry,
      stop_price: s.stop,
      tp_price: s.tp,
      reason: s.reason,
      strategy_type: 'Trend Pullback',
      slippage_bps: 5,
      fee_bps: 10,
      risk_reward_ratio: s.rrr,
      suggested_timeframe: TF,
    }));
  }
};